...
TBridgeYqlPlugin* BridgeCreateYqlPlugin(const TBridgeYqlPluginOptions* bridgeOptions)
{
    ...
    for (auto clusterIndex = 0; clusterIndex < bridgeOptions->AdditionalClusterCount; ++clusterIndex) {
        const auto& additionalCluster = bridgeOptions->AdditionalClusters[clusterIndex];
        additionalClusters[additionalCluster.Cluster] = additionalCluster.Proxy;
    }
    TYqlPluginOptions options{
        .MRJobBinary = TString(bridgeOptions->MRJobBinary),
        .UdfDirectory = TString(bridgeOptions->UdfDirectory),
        .AdditionalClusters = std::move(additionalClusters),
        .YTToken = TString(bridgeOptions->YTToken),
...
TBridgeQueryResult* BridgeRun(TBridgeYqlPlugin* plugin, const char* queryText)
{
    auto* nativePlugin = reinterpret_cast<IYqlPlugin*>(plugin);
    ...
    auto result = nativePlugin->Run(TString(queryText));
