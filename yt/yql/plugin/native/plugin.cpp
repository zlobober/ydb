...
public:
    ...
    TQueryResult GuardedRun(TString impersonationUser, TString queryText)
    {
        auto credentials = MakeIntrusive<NYql::TCredentials>();
        if (YTToken_) {
            credentials->AddCredential("default_yt", NYql::TCredential("yt", "", YTToken_));
        }
    ...
    {
        try {
            return GuardedRun(impersonationUser, queryText);
        } catch (const std::exception& ex) {
            return TQueryResult{
