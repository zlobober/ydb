...
namespace {
void Check(const TPatternGroup& g, const TString& url, const TMaybe<TString>& expectedResult) {
...
Y_UNIT_TEST_SUITE(TPatternGroupTests) {
    Y_UNIT_TEST(All) {
        ...
        g.Add(R"XXX(^https://[\w-]+\.yt\.yandex-team\.ru/)XXX", "success");
        g.Add(R"XXX(^https://(paste|storage|nirvana|grafana|bb|fml)\.yandex-team\.ru/)XXX", "success");
        UNIT_ASSERT(!g.IsEmpty());
