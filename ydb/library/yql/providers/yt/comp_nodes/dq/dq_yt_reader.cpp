...
class TDqYtReadWrapperHttp : public TDqYtReadWrapperBase<TDqYtReadWrapperHttp> {
public:
    ...
    TDqYtReadWrapperHttp(const TComputationNodeFactoryContext& ctx, const TString& clusterName,
        ...
        TType* itemType, const TVector<TString>& tableNames, TVector<std::pair<NYT::TRichYPath, NYT::TFormat>>&& tables, NKikimr::NMiniKQL::IStatsRegistry* jobStats)
            : TDqYtReadWrapperBase<TDqYtReadWrapperHttp>(ctx, clusterName, token, inputSpec, samplingSpec, inputGroups, itemType, tableNames, std::move(tables), jobStats) {}
    ...
    void MakeState(TComputationContext& ctx, NUdf::TUnboxedValue& state) const {
        ...
        state = ctx.HolderFactory.Create<TDqYtReadWrapperBase<TDqYtReadWrapperHttp>::TState>(Specs, ctx.HolderFactory, std::move(rawReaders));
    }
};
...
#ifdef __linux__
    bool useRPC(AS_VALUE(TDataLiteral, callable.GetInput(6))->AsValue().Get<bool>());
    if (useRPC) {
        return new TDqYtReadWrapperBase<TDqYtReadWrapperRPC>(ctx, clusterName, token,
        ...
        return new TDqYtReadWrapperBase<TDqYtReadWrapperHttp>(ctx, clusterName, token,
            NYT::NodeFromYsonString(inputSpec), samplingSpec ? NYT::NodeFromYsonString(samplingSpec) : NYT::TNode(),
            inputGroups, static_cast<TType*>(callable.GetInput(5).GetNode()), tableNames, std::move(tables), jobStats);
    }
#else
