...
public:
    ...
    Value* DoGenerateGetValue(const TCodegenContext& ctx, Value* statePtr, BasicBlock*& block) const {
        ...
        const auto values = new AllocaInst(ArrayType::get(valueType, Representations.size()), 0U, "values", &ctx.Func->getEntryBlock().back());
        ...
        const auto check = new LoadInst(statePtr, "check", block);
...

        {
            ...
            CallInst::Create(addPtr, {stateArg, values}, "", block);
...

        {
            ...
            CallInst::Create(finishPtr, {stateArg}, "", block);
