...
template<typename T>
class TDqYtReadWrapperBase : public TStatefulWideFlowCodegeneratorNode<TDqYtReadWrapperBase<T>> {
using TBaseComputation =  TStatefulWideFlowCodegeneratorNode<TDqYtReadWrapperBase<T>>;
public:
    TDqYtReadWrapperBase(const TComputationNodeFactoryContext& ctx, const TString& clusterName,
        ...
        TType* itemType, const TVector<TString>& tableNames, TVector<std::pair<NYT::TRichYPath, NYT::TFormat>>&& tables, NKikimr::NMiniKQL::IStatsRegistry* jobStats) : TBaseComputation(ctx.Mutables, this, EValueRepresentation::Boxed, EValueRepresentation::Boxed)
    ...
    class TState: public TComputationValue<TState>, public TFileInputState {
    public:
        TState(TMemoryUsageInfo* memInfo, const TMkqlIOSpecs& spec, const THolderFactory& holderFactory,
            TVector<NYT::TRawTableReaderPtr>&& rawReaders)
            : TComputationValue<TState>(memInfo)
            , TFileInputState(spec, holderFactory, std::move(rawReaders), 4, 4_MB)
        {
            SetNextBlockCallback([this]() { Yield_ = true; });
        }
    ...
    ICodegeneratorInlineWideNode::TGenerateResult DoGenGetValues(const TCodegenContext& ctx, Value* statePtr, BasicBlock*& block) const {
        ...
        const auto pointerType = PointerType::getUnqual(ArrayType::get(valueType, Width));
        ...
        const auto self = CastInst::Create(Instruction::IntToPtr, ConstantInt::get(Type::getInt64Ty(context), uintptr_t(static_cast<const TDqYtReadWrapperBase<T>*>(this))), structPtrType, "self", block);
        const auto makeFunc = ConstantInt::get(Type::getInt64Ty(context), GetMethodPtr(&TDqYtReadWrapperBase<T>::MakeState));
        ...
        const auto fetch = CallInst::Create(funcPtr, { stateArg }, "fetch", block);
        ...
        for (ui32 i = 0U; i < Width; ++i) {
            getters.emplace_back([i, placeholder](const TCodegenContext& ctx, BasicBlock*& block) {
                const auto indexType = Type::getInt32Ty(ctx.Codegen->GetContext());
                const auto pointer = new LoadInst(placeholder, (TString("pointer_") += ToString(i)).c_str(), block);
                const auto ptr = GetElementPtrInst::CreateInBounds(pointer, {ConstantInt::get(indexType, 0), ConstantInt::get(indexType, i)}, (TString("ptr_") += ToString(i)).c_str(), block);
                const auto load = new LoadInst(ptr, (TString("load_") += ToString(i)).c_str(), block);
                return load;
            });
        }
