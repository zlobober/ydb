...
public:
    ...
    ICodegeneratorInlineWideNode::TGenerateResult DoGenGetValues(const TCodegenContext& ctx, Value* statePtr, Value* currentPtr, BasicBlock*& block) const {
        ...
        const auto placeholder = GetElementPtrInst::CreateInBounds(ctx.GetMutables(), {ConstantInt::get(statusType,  static_cast<const IComputationNode*>(this)->GetIndex() + 1U)}, "placeholder", &ctx.Func->getEntryBlock().back());
        ...
        for (ui32 i = 0U; i < Width; ++i) {
            getters.emplace_back([i, placeholder, pointerType](const TCodegenContext& ctx, BasicBlock*& block) {
                auto& context = ctx.Codegen->GetContext();
                const auto current = new LoadInst(placeholder, (TString("current_") += ToString(i)).c_str(), block);
