...
private:
        ...
        const TNodeMap<ui32>& commitEpochs, TNodeMap<THashSet<TString>>& tableWritesBeforeCommit)
    {
        ...
        for (auto& commitNode: commits) {
            ...
            THashMap<TString, TExprNode::TPtr> writesByTable;
            ...
            for (auto write: writes) {
                ...
                writesByTable[tableInfo.Name] = write.Ptr();
            }
            for (auto& w: writesByTable) {
                // Skip already processed writes
                if (!NYql::HasSetting(*w.second->Child(4), EYtSettingType::Initial)) {
                    w.second->ChildRef(4) = NYql::AddSetting(*w.second->Child(4), EYtSettingType::Initial, {}, ctx);
                }
            }
            if (!tableNames.empty()) {
