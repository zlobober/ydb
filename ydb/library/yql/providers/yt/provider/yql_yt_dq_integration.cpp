        ...
        if (auto maybeRead = TMaybeNode<TYtReadTable>(&node)) {
            ...
            for (auto section: maybeRead.Cast().Input()) {
                ...
                for (const auto& path: section.Paths()) {
                    if (!path.Table().Maybe<TYtTable>()) {
                        AddErrorWrap(ctx, node.Pos(), "non-table path");
                        return Nothing();
                    } else {
                        groupIdPathInfos.back().emplace_back(new TYtPathInfo(path));
                        auto pathInfo = groupIdPathInfos.back().back();
                        auto tableInfo = pathInfo->Table;

                        Y_ENSURE(tableInfo);
                        Y_ENSURE(tableInfo->Stat);
