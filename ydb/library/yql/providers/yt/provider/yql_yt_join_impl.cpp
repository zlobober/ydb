...
TYtJoinNodeOp::TPtr ImportYtEquiJoinRecursive(const TVector<TYtJoinNodeLeaf::TPtr>& leaves, const TYtJoinNodeOp* parent,
    THashSet<TString>& drops, const TExprNode* joinTree)
{
    TYtJoinNodeOp::TPtr result(new TYtJoinNodeOp());
    result->Parent = parent;
    result->JoinKind = joinTree->Child(0);
    result->LeftLabel = joinTree->Child(3);
    result->RightLabel = joinTree->Child(4);
    result->LinkSettings = GetEquiJoinLinkSettings(*joinTree->Child(5));

    const TStringBuf joinKind = result->JoinKind->Content();

    THashSet<TString> leftKeys;
    ...
    if (joinTree->Child(1)->IsAtom()) {
        result->Left = FindYtEquiJoinLeaf(leaves, joinTree->Child(1)->Content());
    } else {
        result->Left = ImportYtEquiJoinRecursive(leaves, result.Get(), drops, joinTree->Child(1));
    }

    if (joinTree->Child(2)->IsAtom()) {
        result->Right = FindYtEquiJoinLeaf(leaves, joinTree->Child(2)->Content());
    } else {
        result->Right = ImportYtEquiJoinRecursive(leaves, result.Get(), drops, joinTree->Child(2));
    }

    result->Scope.insert(result->Scope.end(), result->Left->Scope.begin(), result->Left->Scope.end());
    result->Scope.insert(result->Scope.end(), result->Right->Scope.begin(), result->Right->Scope.end());
    return result;
}

TYtJoinNodeOp::TPtr CalcYtEquiJoinConstraints(const std::vector<TYtJoinNodeLeaf::TPtr>& leaves, const TYtJoinNodeOp* parent,
    const TExprNode& joinTree, TExprContext& ctx)
{
    TYtJoinNodeOp::TPtr result(new TYtJoinNodeOp());
    result->Parent = parent;
    ...
    bool leftUnique, bool rightUnique, ui64 leftSize, ui64 rightSize)
{
    ...
    auto leftNotFat = leftUnique || op.LinkSettings.LeftHints.contains("unique") || op.LinkSettings.LeftHints.contains("small");
    auto rightNotFat = rightUnique || op.LinkSettings.RightHints.contains("unique") || op.LinkSettings.RightHints.contains("small");
    ...
    outInfo.RowSpec->SetConstraints(equiJoin.Ref().GetConstraintSet());
    outInfo.SetUnique(equiJoin.Ref().GetConstraint<TDistinctConstraintNode>(), pos, ctx);
    ...
    if (!isCrossJoin && state->Configuration->JoinCommonUseMapMultiOut.Get().GetOrElse(DEFAULT_JOIN_COMMON_USE_MULTI_OUT)) {
        ...
        if (joinType->IsAtom({"Left", "Full", "Exclusion"})) {
        ...
        if (joinType->IsAtom({"Right", "Full", "Exclusion"})) {
...
TYtJoinNodeOp::TPtr ImportYtEquiJoin(TYtEquiJoin equiJoin) {
    TVector<TYtJoinNodeLeaf::TPtr> leaves;
    TMaybeNode<TExprBase> world;
    for (size_t i: xrange(equiJoin.Input().Size())) {
        TYtJoinNodeLeaf::TPtr leaf = MakeIntrusive<TYtJoinNodeLeaf>(equiJoin.Input().Item(i),
                                                                    TMaybeNode<TCoLambda>(equiJoin.Ref().ChildPtr(joinFixedArgsCount + i)));
        ...
        if (leaf->Label->IsAtom()) {
            ...
            for (auto& x : leaf->Label->Children()) {
    ...
    auto renameMap = LoadJoinRenameMap(equiJoin.JoinOptions().Ref());
    THashSet<TString> drops;
    for (auto& [column, renames] : renameMap) {
        if (renames.empty()) {
            drops.insert(ToString(column));
        }
    }

    return ImportYtEquiJoinRecursive(leaves, nullptr, drops, equiJoin.Joins().Raw());
}

TConstraintSet CalcYtEquiJoinConstraints(TYtEquiJoin equiJoin, TExprContext& ctx) {
    std::vector<TYtJoinNodeLeaf::TPtr> leaves;
    for (const auto i : xrange(equiJoin.Input().Size())) {
        ...
        if (leaf->Label->IsAtom()) {
            ...
            leaf->Scope.reserve(leaf->Label->ChildrenSize());
    ...
    const auto root = CalcYtEquiJoinConstraints(leaves, nullptr, equiJoin.Joins().Ref(), ctx);
